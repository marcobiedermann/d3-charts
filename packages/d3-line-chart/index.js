...
class D3LineChart {
  ...
  init() {
    const { element, width, height, margin, axis } = this;
    ...
    this.scaleX = d3.scaleTime().range([0, innerWidth]);
  ...
  renderAxis() {
    ...
    svg.select('.d3-line-chart__axis--x').call(xAxis);

    svg.select('.d3-line-chart__axis--y').call(yAxis);
  }
